...
// 요소를 담은 변수명.addEventListener("이벤트 종류",할일)
mbBtn.addEventListener("click",function(){
    ...
    let isActive = mbBtn.classList.contains("active") // isActive->상태변수
    if(isActive==true) {//모바일 메뉴가 활성화되어있는 상태
        // 클래스 active 제거
...
mbMainMenu.forEach(function(mainList,idx){
    mainList.addEventListener('click', function(event){
        event.preventDefault(); // a 태그의 link 막아줌
...
// bannerPlay.addEventListener("click",function(){
//     let spanEl = this.querySelector('.banner-play span')
//     // console.log(spanEl)
//     let txtContent = spanEl.textContent
//     // console.log(txtContent)
//     if(txtContent == "pause"){ // 플레이 상태, 멈춰야함
